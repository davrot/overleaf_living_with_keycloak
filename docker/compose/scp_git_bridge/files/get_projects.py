...
def download_project_zip(
    ...
    try:
        ...
        os.makedirs(os.path.join(base_path,  f"{project_id}.git"), exist_ok=True, mode=0o700)
        filename: str = os.path.join(base_path,  f"{project_id}.git", "data.zip")
        # Save the file
        with open(filename, "w+b") as file:
            for chunk in response.iter_content(chunk_size=8192):
                file.write(chunk)

        subprocess.run([f"cd /downloads/{username}/{project_id}.git && /usr/bin/unzip -o data.zip " ], shell=True)
        subprocess.run([f"rm -f /downloads/{username}/{project_id}.git/data.zip" ], shell=True)
        subprocess.run([f"chown -R {username}:hajtex /downloads/{username}/{project_id}.git " ], shell=True)
        subprocess.run([f"chmod -R 0700 /downloads/{username}/{project_id}.git " ], shell=True)

        if not os.path.isdir("/downloads/{username}/{project_id}.git/.git"):
            subprocess.run([f"chmod 0777 /downloads/{username} " ], shell=True)
            subprocess.run([f"chown -R {username}:hajtex /downloads/{username}/{project_id}.git " ], shell=True)
            subprocess.run([f"cd /downloads/{username}/ && sudo -u {username} /usr/bin/git config --global user.email {username}" ], shell=True)
            subprocess.run([f"cd /downloads/{username}/ && sudo -u {username} /usr/bin/git config --global user.name {username}" ], shell=True)
            subprocess.run([f"cd /downloads/{username}/{project_id}.git && sudo -u {username} /usr/bin/git init -q " ], shell=True)

        subprocess.run([f"cd /downloads/{username}/{project_id}.git && sudo -u {username} /usr/bin/git add --all " ], shell=True)
        subprocess.run([f"cd /downloads/{username}/{project_id}.git && sudo -u {username} /usr/bin/git commit -m 'by HajTex' " ], shell=True)
        subprocess.run([f"chown -R {username}:hajtex /downloads/{username}/{project_id}.git " ], shell=True)
        subprocess.run([f"chmod -R 0755 /downloads/{username}/{project_id}.git " ], shell=True)
        subprocess.run([f"chmod 0755 /downloads/{username} " ], shell=True)
